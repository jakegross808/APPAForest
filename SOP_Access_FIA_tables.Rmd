...
at <- getFIA(states = c('CT', 'GA', 'ME', 'MD', 'MA', 'NH', 
    'NJ', 'NY', 'NC', 'PA', 'TN', 'VT', 'VA'), nCores = cores,dir = './FIA/', common = TRUE) 
```

This is a large amount of data so this step will be slow. You can also choose to load in the downloaded data directly into the R environment with the agrument `r load = TRUE`, however, since this is a large database if all data is too large to fit in memory, use `r load = FALSE` and load the data as a "Remote.FIA.Database" with `r `readFIA` as described in the next step.
...
Each of the following subsections illustrates how to use the variety of functions in `rFIA` to calculate these metrics and provides a decription of their tabular outputs.
