...
You can access the most recent `rFIA` package release with this command: `devtools::install_github("hunter-stanke/rFIA")`
...
levels(as.factor(eco$SUBSECTI_1)) # Subsection names not unique - there are two different "Northern Piedmont"!!
...
Spatially, the `mask = eco` argument below selects only the `at` FIA plots which fall within the perimeter of the `eco` spatial extent (i.e. the outer boundary of the HUC10 Shell polygons).
...
## Using mask option in clipFIA

```{r intersect, include = TRUE, echo=TRUE, warning=FALSE, comment= NA, , eval= FALSE}
# returns all FIA inventories within the APPA ecoregion mask
at_FIA <- clipFIA(at, mask = eco, matchEval= FALSE, mostRecent = FALSE, nCores = cores)
```

<br>

## Access the most recent inventory for all APPA States in FIADB

Set `mostRecent = TRUE`

```{r intersect1, include = TRUE, echo=TRUE, warning=FALSE, comment= NA, , eval= FALSE}
# returns the most recent inventory for all states (mask = NULL) in the `at` database
...
Set `matchEval = TRUE` to return subset of data for which there are matching reporting years across states. Only useful if db contains multiple state subsets of the FIA database.
...
# save as .rds for APPAForestHealth
saveRDS(ss, file = "C:/Users/JJGross/Documents/R_projects/APPAForesthealthReport/summary_data/ss.rds")
```
...
-   **Description:** Estimates of species diversity indices for live trees (DBH \>= 12.7 cm) at alpha, beta, and gamma levels.
...
-   **Data fields:**
    ...
    -   *Eh_a*: estimate of Shannon's Equitability Index for live trees at the alpha (stand) level
    -   *Eh_b*: estimate of Shannon's Equitability Index for live trees at the beta (landscape) level
    -   *Eh_g*: estimate of Shannon's Equitability Index for live trees at the gamma (regional) level
...
Note that stems below 1 inch DBH are not recorded in FIA plots.
...
gmSPlot_spp <- growMort(at_FIA, byPlot= TRUE, bySpecies = TRUE, grpBy = c(STATECD, ECOSUBCD, OWNGRPCD, QA_STATUS,PLOT),  method = "annual", treeDomain = DIA >= 10)

## Recruitment estimates will be null here, as we are excluding stems < 10 inches
# For some reason this is not true, gmSPlot$RECR_TPA does have non-zero values
```
...
# For some reason this is not true, gm$RECR_TPA does have non-zero values
gm <- growMort(at_FIA, polys = eco, nCores = cores, treeDomain = DIA >= 10, method = 'annual')

## To include recruitment
gm_recr_sizeclass <- growMort(at_FIA, polys = eco, nCores = cores, method = 'annual', bySizeClass = TRUE)
```
...
vrS  <- vitalRates(at_FIA, byPlot = TRUE, bySpecies = TRUE, grpBy = c(STATECD, ECOSUBCD, OWNGRPCD, QA_STATUS,PLOT), nCores = cores, treeDomain = DIA >= 10, method = 'annual', totals = )
```
...
```{r eco_vit_net,include = TRUE, echo=TRUE, warning=FALSE, comment= NA, eval= FALSE }
## Net annual change
vr_net <- vitalRates(at_FIA,
                  treeType = 'all', # "all" indicates net growth
                  grpBy = c(STATECD, ECOSUBCD, OWNGRPCD), # Grouping by state, ecological subsection, and ownership
...
```{r eco_vit_live,include = TRUE, echo=TRUE, warning=FALSE, comment= NA, eval= FALSE }
## Annual growth on live trees that remained live
vr_live <- vitalRates(at_FIA,
...
`dwm` produces estimates of down woody material volume (cu.ft/acre) and biomass (tons/acre). As defined by FIA, down woody material includes dead organic materials (resulting from plant mortality and leaf turnover) and fuel complexes of live shrubs and herbs. To maintain relevance for forest fuels management, we report estimates grouped by fuel lag-time classes.

-   **1HR fuels**: small, fine woody debris
-   **10HR fuels**: medium, fine woody debris
-   **100HR fuels**: large, fine woody debris
-   **1000HR fuels**: coarse woody debris and slash piles
-   **duff**: O horizon; all unidentifiable organic material above mineral soil, beneath litter
-   **litter**: identifiable plant material which is downed and smaller than 10HR fuel class (1HR class includes standing herbaceous material).

See Woodall et al. (2007) for further definitions of fuel lag-time classes.
...
dw <- dwm(at_FIA, polys = eco, nCores = cores, method = 'annual')
```
